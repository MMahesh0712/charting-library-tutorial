...
export interface WatchlistContextValue {
  ...
  setWatchlistLoading: Dispatch<SetStateAction<boolean>>;
  ...
  toggleWatchlistFavorite: (id: string, emoji?: string | null) => void;
  clearWatchlist: (id: string) => void;
  copyWatchlist: (id: string, newName: string) => string | null;
  ...
  renameSection: (oldTitle: string, newTitle: string) => void;
  deleteSection: (sectionTitle: string) => void;

  // Import/Export
  exportWatchlist: (id: string) => number | undefined;
  importSymbols: (symbols: WatchlistSymbol[], id: string) => number;
}
 ...
 */
const migrateWatchlistData = (): WatchlistsState => {
  const newData = getJSON<WatchlistsState | null>(STORAGE_KEYS.WATCHLISTS, null);
  ...
  const oldData = getJSON<(WatchlistSymbol | string)[] | null>(STORAGE_KEYS.WATCHLIST, null);
  ...
  if (oldData && Array.isArray(oldData) && oldData.length > 0) {
    ...
    return {
      lists: [
        {
          ...DEFAULT_WATCHLIST,
          symbols: oldData.map((s) =>
            typeof s === 'string' ? { symbol: s, exchange: 'NSE' } : s
          ),
        },
      ],
...
export function WatchlistProvider({ children }: WatchlistProviderProps) {
  ...
  const [watchlistsState, setWatchlistsState] = useState<WatchlistsState>(migrateWatchlistData);
  ...
  const activeWatchlist = useMemo(
    () =>
      watchlistsState.lists.find((wl) => wl.id === watchlistsState.activeListId) ||
  ...
  // Rename watchlist
  const renameWatchlist = useCallback((id: string, newName: string): void => {
    setWatchlistsState((prev) => ({
      ...prev,
  ...
  // Delete watchlist
  const deleteWatchlist = useCallback((id: string): void => {
    setWatchlistsState((prev) => {
      ...
      return {
        lists: newLists,
        activeListId:
          prev.activeListId === id ? newLists[0]?.id || 'wl_default' : prev.activeListId,
      };
    });
  }, []);

  // Switch active watchlist
  const switchWatchlist = useCallback((id: string): void => {
    setWatchlistsState((prev) => ({ ...prev, activeListId: id }));
  }, []);

  // Toggle favorite status
  const toggleWatchlistFavorite = useCallback((id: string, emoji: string | null = null): void => {
    setWatchlistsState((prev) => ({
      ...prev,
  ...
  const clearWatchlist = useCallback((id: string): void => {
    setWatchlistsState((prev) => ({
      ...prev,
      lists: prev.lists.map((wl) =>
  ...
  const addSymbol = useCallback((symbol: string, exchange: string = 'NSE'): void => {
    setWatchlistsState((prev) => {
  ...
  const removeSymbol = useCallback((symbol: string, exchange: string | null = null): void => {
    setWatchlistsState((prev) => ({
      ...prev,
      lists: prev.lists.map((wl) =>
        wl.id === prev.activeListId
  ...
  const reorderSymbols = useCallback((newItems: (WatchlistSymbol | string)[]): void => {
    ...
    setWatchlistData((prev) => {
      const dataMap = new Map(
        prev.map((item) => [`${item.symbol}-${item.exchange || 'NSE'}`, item])
      );
      return newItems
  ...
  const addSection = useCallback((sectionTitle: string, index: number): void => {
    setWatchlistsState((prev) => {
      const activeList = prev.lists.find((wl) => wl.id === prev.activeListId);
      if (!activeList) return prev;
  ...
  const toggleSection = useCallback((sectionTitle: string): void => {
    setWatchlistsState((prev) => {
      const activeList = prev.lists.find((wl) => wl.id === prev.activeListId);
      if (!activeList) return prev;
  ...
  // Rename section
  const renameSection = useCallback((oldTitle: string, newTitle: string): void => {
    setWatchlistsState((prev) => {
      const activeList = prev.lists.find((wl) => wl.id === prev.activeListId);
      if (!activeList) return prev;
  ...
  // Delete section
  const deleteSection = useCallback((sectionTitle: string): void => {
    setWatchlistsState((prev) => {
      const activeList = prev.lists.find((wl) => wl.id === prev.activeListId);
      if (!activeList) return prev;
  ...
  const exportWatchlist = useCallback(
    (id: string): number | undefined => {
      ...
      const symbols = (watchlist.symbols || []).filter(
  ...
  const importSymbols = useCallback((symbols: WatchlistSymbol[], id: string): number => {
